...
export function Reveal({ children, className = "", delay = 0 }: { children: ReactNode; className?: string; delay?: number }) {
  ...
  return <motion.div className={className} initial={reduced ? false : { opacity: 0, y: 22 }} whileInView={reduced ? undefined : { opacity: 1, y: 0 }} viewport={{ once: true, margin: "-70px" }} transition={{ duration: .55, delay, ease: [0.22, 1, 0.36, 1] }}>{children}</motion.div>;
}
...
export function PrimaryLink({ children, href = "#contact", small = false }: { children: ReactNode; href?: string; small?: boolean }) {
  return <a className={`button primary ${small ? "small" : ""}`} href={href}>{children}<ArrowRight aria-hidden="true" size={small ? 16 : 18} /></a>;
}

export function SecondaryLink({ children, href }: { children: ReactNode; href: string }) {
  return <a className="button secondary" href={href} target={href.startsWith("http") ? "_blank" : undefined} rel={href.startsWith("http") ? "noreferrer" : undefined}>{children}</a>;
}
